...
interface RequestedInscriptionEmailProps {
  inscricao: Player;
  torneioTitle: string;
}
...
export default function RequestedInscriptionEmail({
  inscricao,
  torneioTitle,
}: RequestedInscriptionEmailProps) {
  ...
  return (
    <EmailLayout
      preview={`Sua inscrição no torneio ${torneioTitle} foi recebida`}
      titulo="Inscrição solicitada"
    >
      <Text style={paragrafo}>
        Recebemos sua inscrição! Ela será analisada pelo árbitro e você receberá um
        novo email assim que ela for confirmada. Confira abaixo os dados enviados.
      </Text>
